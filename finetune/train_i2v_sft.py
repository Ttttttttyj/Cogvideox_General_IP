...
from models.i2v_ip_transformer import GeneralIP_CogVideoXTransformer3DModel
from models.i2v_ip_pipeline import GeneralIP_Pipeline
...
class VideoDataset(Dataset):
    ...
    def process_ref_image_data(self,path):
        ...
        ref_image = cv2.resize(ref_image, (self.width, self.height))
...
def main(args):
    ...
    # for param in transformer.patch_embed.parameters():
    ...
    transformer_sft_parameters = list(filter(lambda p: p.requires_grad, transformer.parameters()))


    with open("train_i2v_params_sft.txt","w") as file:
        for name, param in transformer.named_parameters():
            if param.requires_grad:
                file.write(f" Train Parameter name: {name}\n")

    # Optimization parameters
    transformer_parameters_with_lr = {"params": transformer_sft_parameters, "lr": args.learning_rate}
        ...
        for epoch in range(first_epoch, args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(models_to_accumulate):
                    ...
                    ref_images = batch["instance_ref_image"].to(dtype=weight_dtype) # [B, 1, C, H, W]
                    videos = videos.permute(0, 2, 1, 3, 4)
                    video_latents = encode_video(videos).to(dtype=weight_dtype) #[2,13,16,60,90]
                    image_latents = process_image(ref_images).to(dtype=weight_dtype) #[2,1,16,60,90]
                    ...
                    noisy_model_input = torch.cat([noisy_video_latents, image_latents], dim=2) # [B,13,32,60,90]
